...
test("storage: formatIdeaArtifact produces parseable JSON metadata followed by body", () => {
...
test("storage: formatIdeaArtifact → parseIdeaArtifact round-trips an artifact", () => {
  const raw = formatIdeaArtifact(SAMPLE);
...
test("storage: parseIdeaArtifact handles body containing literal closing brace }", () => {
...
test("storage: parseIdeaArtifact returns undefined for malformed input", () => {
...
test("storage: normalizeIdeaId returns cfcb8ede for IDEA-cfcb8ede", () => {
  assert.equal(normalizeIdeaId("IDEA-cfcb8ede"), "cfcb8ede");
});

test("storage: normalizeIdeaId returns cfcb8ede for cfcb8ede", () => {
  assert.equal(normalizeIdeaId("cfcb8ede"), "cfcb8ede");
});

test("storage: normalizeIdeaId returns cfcb8ede for IDEA-CFCB8EDE (case insensitive)", () => {
  assert.equal(normalizeIdeaId("IDEA-CFCB8EDE"), "cfcb8ede");
});

test("storage: normalizeIdeaId returns undefined for legacy artifact prefix", () => {
  assert.equal(normalizeIdeaId(`TO${"DO"}-cfcb8ede`), undefined);
});

test("storage: normalizeIdeaId returns undefined for garbage", () => {
  assert.equal(normalizeIdeaId("garbage"), undefined);
});

test("storage: normalizeIdeaId returns undefined for short hex", () => {
  assert.equal(normalizeIdeaId("abc123"), undefined);
});

test("storage: generateIdeaId returns 8 hex chars", () => {
  const id = generateIdeaId();
  assert.match(id, /^[0-9a-f]{8}$/, "should be 8 lowercase hex chars");
});

test("storage: generateIdeaId produces different values on successive calls", () => {
  const ids = new Set(Array.from({ length: 10 }, () => generateIdeaId()));
  assert.ok(ids.size > 1, "should produce unique IDs");
});

test("storage: writeIdea followed by readIdea round-trips in a tmp directory", async () => {
...
test("storage: writeIdea is atomic — no temp files survive a successful write", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: readIdea returns undefined for an unknown id", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: readIdea accepts IDEA-prefixed id", async () => {
  const dir = makeTmpDir();
  try {
    await writeIdea(dir, SAMPLE);
...
test("storage: listIdeas returns parseable entries and silently skips malformed files", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: listIdeas returns empty array for empty directory", async () => {
  const dir = makeTmpDir();
  try {
    const list = await listIdeas(dir);
...
test("storage: listIdeas ignores files that don't match the id pattern", async () => {
  const dir = makeTmpDir();
  try {
    const { writeFile } = await import("node:fs/promises");
...
test("storage: getIdeaDir returns <cwd>/docs/ideas in a sandbox not inside a git repo", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: parseIdeaArtifact normalizes legacy status 'done' to 'closed'", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: parseIdeaArtifact tolerates unknown JSON fields without failing", async () => {
  const dir = makeTmpDir();
  try {
    const { writeFile } = await import("node:fs/promises");
...
test("storage: listIdeas returns richer IdeaListEntry shape with tags and createdAt", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: listIdeas filter by status: open returns only open entries", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: listIdeas filter by status: closed returns only closed entries", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: listIdeas filter by status: all returns all entries", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: listIdeas filter by query returns matching entries", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: appendIdeaBody joins with blank line separator when existing body is non-empty", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: appendIdeaBody with empty body uses no separator", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: appendIdeaBody returns undefined for missing id", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: deleteIdea removes the file and returns the deleted record", async () => {
  const dir = makeTmpDir();
  try {
...
test("storage: deleteIdea returns undefined for missing id", async () => {
  const dir = makeTmpDir();
  try {
