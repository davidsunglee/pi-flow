...
export function parseIdeasArgs(args: string): { query: string; status: "open" | "closed" | "all" } {
...
export class IdeaSelectorComponent implements Component {
  ...
  private actions: WorkMenuAction[];

  constructor(
    entries: IdeaListEntry[],
    initialQuery: string,
    host: IdeaSelectorHost,
  ...
  handleInput(data: string): void {
    ...
    // Quick-key branches fire BEFORE confirm so Enter on a highlighted row falls
    // through to "open" rather than these actions. The built-in refine key is
    // fixed; the remaining quick keys come from configured action shortcuts.
  ...
  render(width: number): string[] {
    ...
    lines.push("");
    lines.push(...wrapTextWithAnsi(` ${theme.fg("dim", buildSelectorHint(this.actions))}`, width));
    lines.push("");
    lines.push(...border.render(width));
...
export class IdeaActionMenuComponent implements Component {
  ...
  constructor(idea: IdeaListEntry, host: IdeaActionMenuHost, opts: { hasWork: boolean }) {
    this.idea = idea;
    this.host = host;
    const items = [
...
export interface IdeaWorkSubmenuHost {
  dispatch(action: WorkMenuAction): void;
  back(): void;
  theme: Theme;
  keybindings: KeybindingsManager;
  requestRender(): void;
...
export class IdeaWorkSubmenuComponent implements Component {
  ...
  constructor(idea: IdeaListEntry, actions: WorkMenuAction[], host: IdeaWorkSubmenuHost) {
    this.idea = idea;
    this.host = host;
    ...
    this.list.onSelect = (item) => {
      const action = actions.find((a) => a.name === item.value);
      if (action) host.dispatch(action);
    };
    this.list.onCancel = () => host.back();
  }
...
export function registerIdea(pi: ExtensionAPI, options: ResolveIdeasConfigOptions = {}): void {
  const config = resolveIdeasConfig(options);
  const hasWork = config.actions.length > 0;
  ...
  pi.registerCommand(config.command, {
    ...
    handler: async (args: string, ctx: ExtensionCommandContext) => {
      const { query, status } = parseIdeasArgs(args);
      const dir = await getIdeaDir(ctx.cwd);
      ...
      await ctx.ui.custom<void>((tui, theme, keybindings, done) => {
        ...
        const openWorkSubmenu = (idea: IdeaListEntry) => {
          const work = new IdeaWorkSubmenuComponent(idea, config.actions, {
            dispatch: (action) => {
              closeAndPrefill(substituteIdea(action.prompt, idea.id));
            },
          ...
          const menu = new IdeaActionMenuComponent(idea, {
            dispatchAction: (name) => {
              if (name === "view") {
                ...
                closeAndPrefill(substituteIdea(config.refinePrompt, idea.id));
              } else if (name === "work") {
                if (hasWork) openWorkSubmenu(idea);
        ...
        const selectorHost: IdeaSelectorHost = {
          ...
          dispatch: (action) => {
            if (action.kind === "cancel") {
              ...
              closeAndPrefill(substituteIdea(config.refinePrompt, action.idea.id));
            } else if (action.kind === "action") {
              closeAndPrefill(substituteIdea(action.action.prompt, action.idea.id));
            }
          },
          theme,
          keybindings,
