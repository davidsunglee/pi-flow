...
`pi-flow` is the primary install path for the pi-flow suite. Users install this single package (`npm:pi-flow` or a git-pinned equivalent) and get workflow resources from `pi-flow-core` plus the bundled UX layer from `pi-flow-ux`.

## What it includes

- **Workflow skills and bundled agents** via `pi-flow-core`
- **Slash commands and the `idea` tool** via `pi-flow-core`
- **Helper runner** (`pi-flow helper <id>`, `pi-flow template <id>`) via `pi-flow-core/bin`
- **UX enhancements** — footer extension, working indicator/message, Nord theme, and packaged defaults via `pi-flow-ux`
