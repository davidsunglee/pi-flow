...
7. **Harden the plan against ambiguous fenced examples.** Run:
   ```
   pi-flow helper _shared/plan_fence_hardening --plan "{PLAN_PATH}" --rewrite-in-place
   ```
...
2. **Resolve the primary reviewer dispatch** by running `resolve-model-dispatch.py --model-tier crossProviderModelTiers.capable --agent plan-reviewer --working-dir "{WORKING_DIR}"`.
...
4. **Dispatch `plan-reviewer`** via `subagent_run_serial` using the helper-prepared prompt at `.prompt_path`.
   ...
   - **4a. Resolve the fallback reviewer dispatch.** Run `resolve-model-dispatch.py --model-tier modelTiers.capable --agent plan-reviewer --working-dir "{WORKING_DIR}"`.
...
5. **Validate and parse the review artifact** by running `pi-flow helper refine-plan/validate-and-parse-plan-review --final-message <finalMessage> --expected-path <review_path from Step 3 or 4b> --reviewer-provenance <reviewer_provenance from Step 3 or 4b> --allowed-tiers crossProviderModelTiers.capable,modelTiers.capable --working-dir "{WORKING_DIR}"`. On non-zero exit, map the helper's stderr JSON `failure` field into the existing `reviewer artifact handoff failed: <specific check>` taxonomy and exit. On exit 0, consume `.review_path`, `.verdict`, `.critical_count`, `.important_count`, `.minor_count`, and `.blocking_findings_markdown`. Treat the on-disk file at `.review_path` as the authoritative review and do NOT use `finalMessage` beyond the handoff marker.
