...
- When the exit code is non-zero and no stable identifier can be extracted, you MUST record exactly one composite `NON_RECONCILABLE_FAILURES` entry naming the failure mode. A non-zero exit with BOTH buckets empty is a protocol violation and is rejected by the parser.
