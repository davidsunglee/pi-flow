...
Run `pi-flow helper define-spec/detect-mux-backend` (passing `--user-input <slash-command-text>` when the user invoked the skill with arguments). Parse the JSON output. Print the returned `status_message` to the user as the informational status line. Route on `branch`: `mux` → Step 3a; `inline` → Step 3b. The runtime probe rules (eight precedence rules byte-equal with `pi-extension/subagents/cmux.ts` + `backends/select.ts`) and the user-input override substring set are encoded in the helper; see its `--help` for the complete contract. Do NOT prompt the user during probing.
...
Run `pi-flow helper _shared/resolve-model-dispatch --tier capable --agent spec-designer`. On non-zero exit, surface the stderr message byte-equal per [`skills/_shared/model-tier-resolution.md`](../_shared/model-tier-resolution.md) and stop. Do not dispatch. Do not fall back to a CLI default.
